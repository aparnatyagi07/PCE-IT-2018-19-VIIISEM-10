...
class Database{
	...
	private $db_user = "userbuilder";  // Change as required
	private $db_pass = "user@#$789";  // Change as required
	private $db_name = "db_ddgroup_stone";	// Change as required
	...
	private $db_name = "db_trips";	// Change as required
	...
	private $results = array(); // Any results from a query will be stored here
    	...
    	// Check to see if the table exists
    	 if($this->tableExists($table)){
            ...
            if($ins = $this->myconn->query($sql)){
            	array_push($this->results,$this->myconn->insert_id);
                return true; // The data has been inserted
            }else{
            	array_push($this->results,$this->myconn->error);
	...
	public function checkIfTimedOut()
	{
		$current = time();// take the current time
		$diff = $current - $_SESSION['loggedCustomerAt'];
		if($diff > $_SESSION['timeCustomerOut'])
		{
			return true;
		}
		else
	...
	public function checkIfAdminTimedOut()
	{
		$current = time();// take the current time
		$diff = $current - $_SESSION['loggedAdminAt'];
		if($diff > $_SESSION['timeAdminOut'])
		{
			return true;
		}
		else
	...
	public function checkIfSellerTimedOut()
	{
		$current = time();// take the current time
		$diff = $current - $_SESSION['loggedSellerAt'];
		if($diff > $_SESSION['timeSellerOut'])
		{
			return true;
		}
		else
	...
	public function getPassPreValue()
	{
		return 'MMCPB_';
	}
	
	public function getPassPostValue()
	{
		return '_MMCPA';
	}
} 
